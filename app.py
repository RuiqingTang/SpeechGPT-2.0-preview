...
def parse_args():
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="SpeechGPT-2.0-preview-7B/",
    )
    parser.add_argument(
        "--codec_ckpt_path",
        default="SpeechGPT-2.0-preview-Codec/sg2_codec_ckpt.pkl",
        type=str,
    )
    parser.add_argument(
...
app = Flask(__name__)
app.config['MAX_CONTENT_LENGTH'] = 100 * 1024 * 1024  # 设置最大上传文件大小（100MB）
...
def configure_app():
    global inference, greeting_audio
...
def process_audio():
    ...
    # 保存上传的文件到临时文件
    temp_directory = 'temp'
    input_path = os.path.join(temp_directory, file.filename)
    file.save(input_path)

    try:
        # 调用 inference forward 方法生成音频
        task = "thought"
        mode = "s2s"
        ...
        if audio_output:
            ...
            return jsonify({"error": "No speech generated."}), 500
    except Exception as e:
        return jsonify({"error": str(e)}), 500

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=10936, debug=True)